...
M.search_replace = function(pattern)
	...
	local left_keypresses =
		string.rep("\\<Left>", string.len(config.options["default_replace_quick_fix_options"]) + shift)
	vim.cmd(
		':call feedkeys(":cfdo %s/'
			.. util.double_escape(pattern)
			.. "//"
			.. config.options["default_replace_quick_fix_options"]
			.. left_keypresses
			.. '")'
	)
end
...
M.visual_charwise_selection = function()
	...
	local left_keypresses =
		string.rep("\\<Left>", string.len(config.options["default_replace_quick_fix_options"]) + 1)
